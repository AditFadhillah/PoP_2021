...
type Drone (position:int*int, destination:int*int, speed: int) = 
    ...
    member this.Fly = 
        ...
        if crashed = false && Length = 0 then
...
type Airspace (sdrones : Drone list) =
    ...
    member this.WillCollide (minute: int) : (Drone*Drone) list =
        ...
        for x in 0..((60 * minute) - 1) do
            this.FlyDrones 
            for i in [0..(drones.Length - 1)] do
                for j in [i+1..(drones.Length - 1)] do
                    if((this.DroneDist(drones.[i])(drones.[j])) < 500 && not ((drones.[i].isCrashed) || (drones.[j].isCrashed))) then
                        CollidedDrone <- CollidedDrone @ [drones.[i], drones.[j]]
                        (drones.[i].setCrashed)
                        (drones.[j].setCrashed)
                        printfn "%A %A Collided at: " (drones.[i].getPosition) (drones.[j].getPosition)
        CollidedDrone
